...
- **捕获机制**（D-023）：snapshot=M3dView.active3dView().readColorBuffer()（VP2 必须 img.create(kFloat)→readColorBuffer→convertPixelFormat(kByte)，否则全黑）→MImage.writeToFile 临时 PNG→QImage 降采样→JPEG base64→删文件，含 HUD/选区即"agent 所见"特性，调用前 cmds.refresh(force=True)；preview=cmds.playblast(format=image/compression=png/offScreen=True/viewer=False/showOrnaments=False/widthHeight/percent=100/forceOverwrite=True/editorPanelName 显式钉面板)，相机经 modelPanel 命名参数 API 指定（T-11/D-039 起；lookThru 歧义 API 退场）。
- **修正**（2026-09-20 T-13 审计 R-1）：上条"必须 convertPixelFormat(kByte) 否则全黑"经 Maya 2024 真机证伪——convertPixelFormat 为 2025+ MImage API（2024 om.MImage 无此方法，readColorBuffer 已产可写格式，live 实证同图输出）；MImage 自 2025 起由 maya.api.OpenMaya 移至 maya.api.OpenMayaUI，visual_module 双模块解析+hasattr 守卫（ADR-0018/D-046 轮）。
...
- **证据强度说明**：readColorBuffer 已标 Deprecated（未移除）→捕获路径隔离为可替换单元；kFloat 路径在 2025/2026 仅 API 签名佐证无回归实测；MImage PNG 是否需 verticalFlip 两源分歧→首个真机窗口验证；dcc-mcp-maya 内部截图实现未读出（可选 pip download 审源码）；多 block 逐客户端渲染行为无官方文档→DoD 含 MCP Inspector+Claude Code+Codex 实测。
