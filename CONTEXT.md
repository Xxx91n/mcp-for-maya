...
**注入模块准入判据**:
新 Maya 侧注入模块的立项门槛——三判据全满足才允许新建独立模块：①独立注入时机（懒注入触发点自成节拍）②独立 Maya 侧 API 面（cmds/omui 调用面与既有模块正交）③独立失败域（异常/降级语义不与他模块共享）。域边界=注入边界=文件粒度=失败域粒度；判据防「改巨石不方便」式先例滑梯（ADR-0027）。
